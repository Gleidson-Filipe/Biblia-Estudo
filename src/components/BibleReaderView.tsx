...
export interface BibleReaderViewRef {
  ...
  updateBadges: (noteVerses: number[], corrVerses: number[], groupNoteVerses?: number[], groupCorrVerses?: number[], savedVerses?: number[]) => void;
...
const BibleReaderView = forwardRef<BibleReaderViewRef, Props>(({ style, isDark, onVersePress, onVerseNumPress, onVerseComparePress, onInterlinearWordPress, onInterlinearDismiss }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    updateBadges: (noteVerses: number[], corrVerses: number[], groupNoteVerses: number[] = [], groupCorrVerses: number[] = [], savedVerses: number[] = []) => {
      const handle = findNodeHandle(nativeRef.current);
      if (handle) {
        UIManager.dispatchViewManagerCommand(handle, 'updateBadges', [JSON.stringify(noteVerses), JSON.stringify(corrVerses), JSON.stringify(groupNoteVerses), JSON.stringify(groupCorrVerses), JSON.stringify(savedVerses)]);
      }
    },
